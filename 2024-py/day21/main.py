...
def find_shortest_sequence(code: str, proxies: int, keypad_type: KeypadType = KeypadType.NUMPAD) -> int:
    ...
    if proxies == 0:
        ...
        return min(
            sum(find_shortest_sequence(dircode, proxies - 1, KeypadType.DIRPAD) for dircode in sequence) for sequence in
            sequences)
...
if __name__ == '__main__':
    codes = list(line.strip() for line in open("i.txt").readlines())
    part1(codes)
    part2(codes)
